...
use App\Data\Notifications\Returns\ReturnApprovedData;
...
class CustomerReturnApprovedNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        return (new MailMessage)
            ->subject("تمت الموافقة على طلب الإرجاع #{$this->returnOrder->return_number}")
            ->greeting("مرحباً {$notifiable->name}")
            ->line("تمت الموافقة على طلب الإرجاع رقم {$this->returnOrder->return_number} للطلب رقم {$orderNumber}.")
            ->line('سيتم تزويدك بالتحديثات القادمة فور توفرها.')
    ...
    public function toArray(object $notifiable): array
    {
        ...
        return (new ReturnApprovedData(
            return_number: $this->returnOrder->return_number,
            order_number: $orderNumber,
            title: 'تمت الموافقة على الإرجاع',
            message: "تمت الموافقة على طلب الإرجاع رقم {$this->returnOrder->return_number} للطلب رقم {$orderNumber}.",
